...
for f in `find . -type f` ; do
	...
	sed "s=/usr/local/nagios/var/rw/=/var/spool/nagios/=; \
		s=/usr/local/nagios/libexec/eventhandlers/=%{_prefix}/lib64/nagios/plugins/eventhandlers=; \
...
# install CGIs
cd contrib
make INSTALL=install DESTDIR=${RPM_BUILD_ROOT} INSTALL_OPTS="" COMMAND_OPTS="" CGIDIR=%{_prefix}/lib64/nagios/cgi install
...
install -d -m 0755 ${RPM_BUILD_ROOT}%{_prefix}/lib64/nagios/eventhandlers
for file in * ; do
    test -f $file && install -m 0755 $file ${RPM_BUILD_ROOT}%{_prefix}/lib64/nagios/eventhandlers && rm -f $file
done
cd ../..
...
%files www
%defattr(755,root,root)
%dir %{_prefix}/lib64/nagios/cgi
%{_prefix}/lib64/nagios/cgi/*
